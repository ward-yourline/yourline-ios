...
enum OrdersViewSection: Int, CaseIterable {
    case summary
    case orders
}
class OrdersViewModel: OrdersViewModelling {
    var numberOfSections: Int { return OrdersViewSection.allCases.count }
    
    private weak var view: OrdersViewing?
    private let router: OrdersRouting
    private lazy var service = MockWebservice()
    
    private var model: OrdersModel?
    
    required init(view: OrdersViewing?, router: OrdersRouting) {
